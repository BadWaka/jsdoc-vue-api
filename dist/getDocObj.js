  ...
  jsdocObj.forEach((commentItem, commentIndex) => {
    ...
    if (commentItem.memberof && commentItem.memberof.indexOf('module.exports.props') !== -1) {
      ...
      if (commentItem.memberof === 'module.exports.props') {
        let metaCodeValue = JSON.parse(commentItem.meta.code.value); // 获取默认值

        let defaultValue = metaCodeValue.default; // 获取是否必须

        let required = metaCodeValue.required; // 获取子属性
        ...
        docObj.props[commentItem.name] = propObj;
          ...
          let propName = commentItem.longname.replace('module.exports.props.', '').replace('.type', ''); // 把类型赋值给属性
